...
def main():
    for row in data:
        if all(not is_han(c) for c in row):
            sure_non_matches.append("NOH  {}".format(row))
            continue
    ...
    print("{} sure non-matches".format(len(sure_non_matches)))
    print("{} regex non-matches".format(len(non_matches)))
    print("{} alternatives non-matches".format(len(with_alternatives)))
    print("{} length non-matches".format(len(length_mismatches)))
